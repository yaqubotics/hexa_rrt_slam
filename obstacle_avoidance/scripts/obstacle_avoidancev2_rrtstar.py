	...
	while(True):
		if(rospy.Time.now()-last_request > rospy.Duration(0.1)):
			...
			if(not LineOfSight(walls,(cur_pose.pose.position.x,cur_pose.pose.position.y),(poses.pose.position.x,poses.pose.position.y))):
				...
				global_los = False
				poses.pose.position.x = cur_pose.pose.position.x
				poses.pose.position.y = cur_pose.pose.position.y
...
def main(args):
	...
	map_sub=rospy.Subscriber("/map",OccupancyGrid,queue_size=2,callback=map_cb)
	...
	while(not rospy.is_shutdown()):
		...
		if((rospy.Time.now()-last_request_rrt > rospy.Duration(0.1)) or (init_RRT)) and (map_flag):
			...
			if(dist((cur_pose.pose.position.x,cur_pose.pose.position.y),(goal.pose.position.x,goal.pose.position.y)) < 2.0):
				...
				print "Calculate RRT"
				path_found = False
				x_init = (cur_pose.pose.position.x,cur_pose.pose.position.y)
				x_goal = (goal.pose.position.x,goal.pose.position.y)
				r = 1
				prc = 0.1
				max_samples = 1024
				map_size_per_2 = map_size/2
				Q = np.array([(8, 4)])
				rewire_count = 32
				X_dimensions = np.array([(-map_size_per_2, map_size_per_2), (-map_size_per_2, map_size_per_2)])
