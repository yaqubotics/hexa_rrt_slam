...
def publish_pose_thread():
	...
	global uav_marker0_publisher,uav_marker0,cur_pose0
	global rrt_marker,rrt_marker_publisher
	init_position = False
	...
	while(True):
		if(rospy.Time.now()-last_request > rospy.Duration(0.2)):
			...
			uav_marker0.pose = cur_pose0.pose
			uav_marker0_publisher.publish(uav_marker0)
			uav_goal_marker_publisher.publish(uav_goal_marker)
...
def main(args):
	global list_current_position, list_current_velocity, current_position, usingvelocitycontrol,usingpositioncontrol,xvel,yvel,state,cur_pose,cur_pose0,list_error,target
	...
	global cost_map,cost_map_publisher,uav_marker0,uav_marker0_publisher
	goal_changed_start = True
	global_los = True
	goal_changed = True
	replanning_rrt = True
	...
	print "test1"
	state_sub=rospy.Subscriber("/mavros/state", State,queue_size=10,callback=state_cb)
	map_sub=rospy.Subscriber("/map",OccupancyGrid,queue_size=10,callback=map_cb)
	local_vel_pub=rospy.Publisher("/mavros/setpoint_velocity/cmd_vel",TwistStamped,queue_size=10)
	local_pos_pub=rospy.Publisher("/mavros/setpoint_position/local",PoseStamped,queue_size=10)
	local_pos_target=rospy.Publisher("/mavros/setpoint_raw/local",PositionTarget,queue_size=10)
	atittude_pub = rospy.Publisher("/mavros/setpoint_raw/attitude",AttitudeTarget,queue_size=10)
	thr_pub = rospy.Publisher("/mavros/setpoint_attitude/att_throttle",Float64,queue_size=10)
	...
	position_sub = rospy.Subscriber("/mavros/local_position/pose",PoseStamped,queue_size = 10,callback=position_single_cb)
	...
	position_slam_sub = rospy.Subscriber("/slam_out_pose",PoseStamped,queue_size = 10,callback=position_slam_cb)
	...
	cost_map_publisher = rospy.Publisher('/cost_map',OccupancyGrid,queue_size=10)
	...
	topic = 'uav_marker0'
	uav_marker0_publisher = rospy.Publisher(topic, Marker,queue_size=10)
	uav_marker0 = Marker()
	uav_marker0.header.frame_id = "map"
	uav_marker0.type = uav_marker0.SPHERE
	uav_marker0.action = uav_marker0.ADD
	uav_marker0.scale.x = 0.5
	uav_marker0.scale.y = 0.5
	uav_marker0.scale.z = 0.5
	uav_marker0.color.r = 255.0/255.0
	uav_marker0.color.g = 0.0/255.0
	uav_marker0.color.b = 255.0/255.0
	uav_marker0.color.a = 255.0/255.0
	...
	goal_sub = rospy.Subscriber("/move_base_simple/goal",PoseStamped,queue_size = 10,callback=goal_cb)

	topic = 'sub_goal_marker'
		...
		if((rospy.Time.now()-last_request_rrt > rospy.Duration(0.1)) or (init_RRT)) and (map_flag):
			...
			if(dist((cur_pose.pose.position.x,cur_pose.pose.position.y),(goal.pose.position.x,goal.pose.position.y)) < 2.0):
				...
				if(path_found):
					...
					if(goal_changed) or (dr < 3.0*dist((cur_pose.pose.position.x,cur_pose.pose.position.y),(goal.pose.position.x,goal.pose.position.y)) or (not global_los)): #not global_los) or (len(rrt_path_temp) <= prev_length_rrt_path+5):
