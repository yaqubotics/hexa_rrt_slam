...
class Parameter(object):
	...
	with open('/home/yaqub/yaqub_ws/src/coverage_control_uavs/control_uav/scripts/config.json','r') as f:
		config = json.load(f)
	...
	ws_model = dict()
	#robot radius
	ws_model['robot_radius'] = config["DEFAULT"]["ROBOT_RADIUS"] #*RealScale
	ws_model['robot_clearence'] = config["DEFAULT"]["ROBOT_CLEARENCE"] #*RealScale
	...
	if(UsingObstacle):
		for i in range(num_of_obstacles):
			if(config["OBSTACLE"][repr(i)]["TYPE"] == "CYLINDER"):
				...
				obstacle_pose = config["OBSTACLE"][repr(i)]["POSE"]
				obstacle_size = config["OBSTACLE"][repr(i)]["SIZE"]
				ws_model['rectangle_obstacles'].append([obstacle_pose[0],obstacle_pose[1],obstacle_size[0]*RealScale,obstacle_size[1]])
			else:
	...
	for hole in ws_model['circular_obstacles']:
		for i in range(NumberOfPoints):
			for j in range(NumberOfPoints):
				...
				if (dist < (1.2*hole[2]+ws_model['obs_clearence']+ws_model['robot_radius']/2)):
