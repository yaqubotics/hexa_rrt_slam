...
def main(args):
    ...
    for i in range(grid_size):
        for j in range(grid_size):
               if(listog[j*grid_size+i] == 100):
                   gg = {(x+i,y+j) for (x,y) in g}
                   #walls_temp.add((i,j))
                   walls = walls.union(gg)
    rrt_star = True
    NUMNODES = 50
    map_size = grid_size
    ...
    rrt_marker_color = [(255,0,0),(0,155,0),(0,0,255),(255,255,0)]
    uav_marker_color = [(255,255,0),(0,0,255),(0,155,0),(255,0,0)]
    ...
    for a in range(0,NUM_OF_AGENT):
        ...
        uav_start_marker.type = uav_start_marker.SPHERE
        uav_start_marker.action = uav_start_marker.ADD
        uav_start_marker.scale.x = 2.0#par.ws_model['robot_radius']*2
        uav_start_marker.scale.y = 2.0#par.ws_model['robot_radius']*2
        uav_start_marker.scale.z = 2.0#0.005*par.RealScale
        ...
        voronoi_pcl_publisher = rospy.Publisher('voronoi_pcl_'+repr(a), PointCloud,queue_size=10)
        voronoi_pcl = PointCloud()
        voronoi_pcl.header.frame_id = "map"
        voronoi_pcl.points = []
        voronoi_pcl_list.append(voronoi_pcl)
        voronoi_pcl_publisher_list.append(voronoi_pcl_publisher)
    print "ok1"
    NUM_OF_NODES = 400
    time0 = time.time()
    ...
    diagram4 = GridWithWeights(grid_size, grid_size)
    diagram4.walls = []
    for i in range(grid_size):
        for j in range(grid_size):
            if (i,j) in walls:
    ...
    cost_so_far_list = []
    time0 = time.time()
    for a in range(NUM_OF_AGENT):
        map_cost_of_agent.append(np.ones((grid_size,grid_size))*999)
        start = (agent_pos[a][0]+int(grid_size/2), agent_pos[a][1]+int(grid_size/2))
        cost_so_far = dijkstra_cost(diagram4, start)
        #cost_so_far = a_star_cost(diagram4, start)
        #print came_from
        cost_so_far_list.append(cost_so_far)
        #map_cost_of_agent[a][i,j] = cost_so_far[(i,j)]
    #final_cost = []
    #for a in range(NUM_OF_AGENT):
    #final_cost
    for i in cost_so_far_list[0]:
        aaa = []
        for a in range(NUM_OF_AGENT):
            aaa.append(cost_so_far_list[a][i])
        p32 = Point32()
        p32.x = i[0]-int(grid_size/2)
        p32.y = i[1]-int(grid_size/2)
        p32.z = 0
        voronoi_pcl_list[np.argmin(aaa)-1].points.append(p32)
    print time.time()-time0
    print len(cost_so_far_list[0])
    print len(cost_so_far_list[1])
    print len(cost_so_far_list[2])
    print len(cost_so_far_list[3])
    #print map_cost_of_agent[0]
    ...
    while(not rospy.is_shutdown()):
        ...
        for a in range(0,NUM_OF_AGENT):
            ...
            voronoi_pcl_publisher_list[a].publish(voronoi_pcl_list[a])
        r.sleep()
        #print "bb"
    print "cc"
    try:
