...
def computeCoverage(EstimationStructure,Parameter,Variance,ws_model):
	...
	EstimationStructure.Node = computeMomenta(EstimationStructure.Node,Parameter,EstimationStructure.DensityFunction,ws_model)
...
def computeClassicCoverage(LocalParameter,NoEstimation,TrueFunction):
	...
	for t in range(1,LocalParameter.Niter):
		...
		NoEstimation.Node = computeMomenta(NoEstimation.Node,LocalParameter,TrueFunction,LocalParameter.ws_model)
		for i in range (LocalParameter.NumberOfAgents):
...
def computeEnergyFunction(V,Parameter,C=0):
	E = 0
	for i in range(Parameter.NumberOfAgents):
		E += 0.5*np.dot(V[i],np.transpose(V[i]))#+9.8*Parameter.Altitude+C
	return E
...
def computeMaxLocationModified(EstimationStructure,Node,Parameter,Variance):
	...
	r = Parameter.ws_model['robot_radius']
	c = Parameter.ws_model['robot_clearence']
	mindis = 2*(r+c)
...
def computeMomenta(Node,LocalParameter,DensityFunction,ws_model):
...
def vectorExponential(xAxis,yAxis,inputLocations,rowVector,n,scale,numofagents):
	...
	#rV = rowVector 
	#for i in range(n-numofagents):
	#	rowVector[i,0] = lastkernelrow[i]
	...
	#for i in range(n):
		...
		#print np.exp(-(np.power(xAxis-inputLocations[i,0],2) + np.power(yAxis-inputLocations[i,1],2))/(scale**2))
		#rowVector[i,0] = np.exp(-(np.power(xAxis-inputLocations[i,0],2) + np.power(yAxis-inputLocations[i,1],2))/(scale_2))
		#rV[i,0] = np.exp(-(np.multiply(xAxis-inputLocations[i,0],xAxis-inputLocations[i,0]) + np.multiply(yAxis-inputLocations[i,1],yAxis-inputLocations[i,1]))/(scale_2))
	#print rV
